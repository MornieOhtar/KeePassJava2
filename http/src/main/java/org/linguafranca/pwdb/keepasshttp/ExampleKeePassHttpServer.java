...
public class ExampleKeePassHttpServer {
    private static final String _DEFAULT_DB_FILE = "HttpDatabase.kdbx";
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (Files.notExists(Paths.get(_DEFAULT_DB_FILE))) {
            SimpleDatabase db = new SimpleDatabase();
            db.setName("HTTP Database");
            db.save(creds, new FileOutputStream(_DEFAULT_DB_FILE));
        }

        DatabaseAdaptor adaptor = new DatabaseAdaptor.Default(new File(_DEFAULT_DB_FILE), creds, new HexPwGenerator(10));
